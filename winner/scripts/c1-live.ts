...
import { ACTION, CONTRACT_TAIL, CONTRACT_VERSION, INCIDENT_MAP_TAIL } from "./constants.js";
...
async function main() {
  ...
  if (targetRun.code !== 0) throw new Error(`target setup failed: ${targetRun.stderr.slice(0, 500)}`);
  ...
  const totalDelete = evidence.broker_race.destructive_delete_total;
  const totalTokens = evidence.broker_race.token_minted_total;
  const statuses = brokers.map((item) => item.claim_outcome);
  if (statuses.filter((status) => status === "CLAIM_WON").length !== 1 || statuses.filter((status) => status === "CLAIM_LOST").length !== 1 || totalDelete !== 1 || totalTokens !== 1 || (evidence.final_t3n_authority?.status ?? null) !== "CLOSED") throw new Error("C1 kill condition: effect-safe race did not meet the brutal pass criteria");
