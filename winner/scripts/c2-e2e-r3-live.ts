...
import { adjudicateClosedReplayBrokerResult, closedTerminalAuthorityProjection } from "../c2/closed-replay-adjudication.js";
...
const HISTORICAL_R2_POLICY_FILE = "winner/evidence/C2-E2E-R2-LIVE-POLICY.json";
const HISTORICAL_R2_RETIREMENT_FILE = "winner/evidence/C2-E2E-R2-POLICY-RETIREMENT.json";
const IMPLEMENTATION_PATHS = ["winner/c2", "winner/broker", "winner/contract", "winner/scripts", "winner/tests"];

type JsonObject = Record<string, any>;
type ApiResult = { status: number; body: unknown; text: string; headers: Record<string, string> };
...
async function assertImplementationUnchanged(startingCommit: string, frozenManifest: { sha256: string }): Promise<void> {
  const changed = await runGit(["diff", "--name-only", `${startingCommit}..HEAD`, "--", ...IMPLEMENTATION_PATHS]);
  requireCondition(changed === "", `implementation path changed after R3 freeze: ${changed}`);
  const current = await implementationManifest(startingCommit);
  requireCondition(current.sha256 === frozenManifest.sha256, "implementation manifest changed after R3 freeze");
}
...
async function githubRequest(token: string, route: string, init: RequestInit = {}): Promise<ApiResult> {
  const response = await fetch(`${API}${route}`, {
    ...
    headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json", "X-GitHub-Api-Version": API_VERSION, "User-Agent": "t3n-breakglass-c2-e2e-r3", ...(init.headers ?? {}) },
  });
...
async function appReadiness(): Promise<JsonObject> {
  ...
  const endpoint = await ingressReachability();
  const readiness = evaluateGithubAppWebhookReadiness({
    expected_url: `${EXPECTED_PUBLIC_URL}${WEBHOOK_ROUTE}`,
    app: { http_status: app.status, id: typeof appBody.id === "number" ? appBody.id : null, slug: typeof appBody.slug === "string" ? appBody.slug : null, permissions: appPermissions, events },
    installation: { http_status: installation.status, id: Number.isSafeInteger(Number(installationBody.id)) ? Number(installationBody.id) : null, repository_selection: typeof installationBody.repository_selection === "string" ? installationBody.repository_selection : null, permissions: installationPermissions },
    hook: { http_status: hook.status, url: typeof hookBody.url === "string" ? hookBody.url : null, content_type: typeof hookBody.content_type === "string" ? hookBody.content_type : null, insecure_ssl: typeof hookBody.insecure_ssl === "string" || typeof hookBody.insecure_ssl === "number" ? hookBody.insecure_ssl : null, secret: hookBody.secret },
    tunnel: { ngrok_api_http_status: endpoint.ngrok_api_http_status, public_origin: endpoint.public_url_origin, forwarding_address: endpoint.tunnel_addr, public_route_probe_http_status: endpoint.public_http_status },
    receiver: { listening_locally: endpoint.local_http_status === 404 },
  });
  requireCondition(readiness.valid, `GitHub App webhook readiness failed: ${readiness.reasons.join(", ")}`);
...
async function deliveryHistory(deliveryId: string): Promise<JsonObject> {
  ...
  requireCondition(response.status === 200, `GitHub delivery history failed HTTP ${response.status}`);
  const rows = parseGithubDeliveryList(response.text);
  const match = selectOriginalDelivery(rows, { guid: deliveryId, event: "push", installation_id: Number(INSTALLATION_ID), repository_id: SANDBOX_REPOSITORY_ID });
  requireCondition(response.status === 200 && match, `GitHub delivery ${deliveryId} was not corroborated`);
  return { classification: "GITHUB_DELIVERY_CORROBORATED", http_status: response.status, delivery_id_exact: match.id, guid: match.guid, event: match.event, delivered_at: match.delivered_at, status: match.status, status_code: match.status_code, installation_id: match.installation_id, repository_id: match.repository_id, redelivery: match.redelivery };
}
...
async function generateKeyAndStage(): Promise<{ title: string; publicKey: string; fingerprint: string; privateDigest: string; blobSha: string }> {
  tempKeyDirectory = await mkdtemp(path.join(os.tmpdir(), "t3n-c2-e2e-r3-key-"));
  const privatePath = path.join(tempKeyDirectory, "id_ed25519");
  const publicPath = `${privatePath}.pub`;
  const title = `${R3_E2E_SCHEMA.targetTitlePrefix}${Date.now()}-${randomBytes(6).toString("hex")}`;
  ...
  stagingDirectory = await mkdtemp(path.join(os.tmpdir(), "t3n-c2-e2e-r3-stage-"));
...
async function triggerSecret(bytes: Buffer): Promise<JsonObject> {
  triggerDirectory = await mkdtemp(path.join(os.tmpdir(), "t3n-c2-e2e-r3-trigger-"));
  ...
  const commitSha = await runGit(["commit-tree", treeSha, "-p", BEFORE_SHA, "-m", "C2-E2E-R3 exact disposable credential transition"], directory);
...
async function t3nReadiness(agentKey: string, brokerKey: string): Promise<{ t3n: Awaited<ReturnType<typeof connectTenant>>["t3n"]; nodeUrl: string; readiness: JsonObject }> {
  ...
  const noSuch = `C2-E2E-R3-READINESS-${Date.now()}-${randomBytes(4).toString("hex")}`;
...
async function main(): Promise<void> {
  ...
  const historicalR2PolicyWrapper = await readJson<JsonObject>(HISTORICAL_R2_POLICY_FILE);
  const historicalR2Policy = historicalR2PolicyWrapper.policy as C2PushPolicyV2;
  const historicalPolicies = [historicalB1Policy, historicalR1Policy, historicalR2Policy];
  ...
  const r2Retirement = await readJson<JsonObject>(HISTORICAL_R2_RETIREMENT_FILE);
  ...
  const retiredIds = retiredPolicyIdSet([
    ...
    { policy_id: r2Retirement.policy_id ?? r2Retirement.historical_policy_id, retired: true, retirement_reason: r2Retirement.retirement_reason ?? r2Retirement.failure_classification ?? "historical failed E2E policy", retirement_timestamp: r2Retirement.retirement_timestamp, retirement_evidence_identity: r2Retirement.retirement_evidence_identity },
  ]);
  const policyId = `${R3_E2E_SCHEMA.policyIdPrefix}${Date.now()}-${randomBytes(6).toString("hex")}`;
  ...
  const policyBytes = await writeJson(POLICY_FILE, { artifact: "C2-E2E-R3 LIVE AUTHORITY-BEARING POLICY", registry_identity: policyId, policy: authorityFields });
  const policyFreezeSha = await commitAndPush([POLICY_FILE], `c2: freeze E2E-R3 live policy ${policyId}`);
  await assertImplementationUnchanged(executionHead, frozenImplementationManifest);
  await assertImplementationUnchanged(executionHead, frozenImplementationManifest);
  ...
  const markerSha = await commitAndPush([MARKER_FILE], `c2: attest E2E-R3 policy freeze ${policyId}`);
  await assertImplementationUnchanged(executionHead, frozenImplementationManifest);
  await assertImplementationUnchanged(executionHead, frozenImplementationManifest);
  ...
  requireCondition(c2.classification === "C2_PUSH_SELECTED" && c2.dedupe.status === "NEW" && c2.replayed === false && c2.authority_rederived === true && c2.source_reads === 2, `C2 production pipeline rejected the real transition: ${c2.classification}`);
  requireCondition(c2.create_request.deploy_key_id === target.id, "C2 pipeline did not derive the exact fresh target");
  ...
  const b1Verification = verifyB1Evidence(b1Evidence, verificationContext, R3_B1_SCHEMA);
  ...
  const brokerRun = await mkdtemp(path.join(os.tmpdir(), "t3n-c2-e2e-r3-broker-"));
  ...
  progress("retire-policy-and-replay");
  const policyRetirement = { artifact: "C2-E2E-R3-POLICY-RETIREMENT", policy_id: policyId, deploy_key_id: target.id, terminal_incident_id: derivedRequest.incident_id, terminal_classification: "VERIFIED_ABSENT", retired: true, retirement_timestamp: new Date().toISOString(), retirement_evidence_identity: FINAL_FILE, reason: "C2_E2E_R3_COMPLETED_TARGET_REMOVED" };
  await writeJson(RETIREMENT_FILE, policyRetirement);
  const policyRetirementSha = await commitAndPush([RETIREMENT_FILE], `c2: retire E2E-R3 policy ${policyId}`);
  await assertImplementationUnchanged(executionHead, frozenImplementationManifest);
  const receiptPath = path.join(dedupeDirectory, `${c2.dedupe.key}.json`);
  const receiptShaBefore = sha256(await readFile(receiptPath));
  const replayRequest = { headers: rawHeaders, body: raw };
  let replayObservationAccesses = 0;
  const poisonObservations = {
    get before(): ImmutablePathObservation { replayObservationAccesses += 1; throw new Error("replay accessed before observation"); },
    get after(): ImmutablePathObservation { replayObservationAccesses += 1; throw new Error("replay accessed after observation"); },
  } as { before: ImmutablePathObservation; after: ImmutablePathObservation };
  const c2Replay = await processPushWebhook(replayRequest, webhookSecret, dedupeDirectory, [], poisonObservations, { retiredPolicyIds: new Set([policyId, ...retiredIds]) });
  const receiptShaAfter = sha256(await readFile(receiptPath));
  requireCondition(c2Replay.classification === "C2_PUSH_SELECTED" && c2Replay.replayed === true && c2Replay.receipt_replay === true && c2Replay.authority_rederived === false && c2Replay.source_reads === 0 && c2Replay.dedupe.status === "DUPLICATE_SAME" && c2Replay.incident_id === derivedRequest.incident_id && JSON.stringify(c2Replay.create_request) === JSON.stringify(derivedRequest) && replayObservationAccesses === 0 && receiptShaBefore === receiptShaAfter, "C2 replay did not return the unchanged durable request without source reads");
  ...
  requireCondition(c1ReplayReserve.result !== "WON", "closed remediation replay unexpectedly won a reservation");
  const replayRun = await mkdtemp(path.join(os.tmpdir(), "t3n-c2-e2e-r3-replay-")); const replayBarrier = path.join(replayRun, "barrier.json"); const replayReady = path.join(replayRun, "ready.json"); const replayResult = path.join(replayRun, "result.json");
  const replayBrokerEnvironment = childEnvironment({ EFFECT_BROKER_T3N_API_KEY: brokerKey, EFFECT_BROKER_DID: BROKER_DID, C1_BARRIER_FILE: replayBarrier, C1_PROPOSALS_COMPLETE_FILE: path.join(replayRun, "proposals.json"), C1_OPERATOR_DID: OPERATOR_DID, C1_EXPECTED_CLAIM_VERSION: String(object(closed.detail).effect_claim_version ?? 1) });
  const replayPromise = runChild(path.join(root, "winner/broker/run.ts"), [derivedRequest.incident_id], { ...replayBrokerEnvironment, C1_READY_FILE: replayReady, C1_RESULT_FILE: replayResult, C1_CONTENDER_ID: "replay" });
  ...
  const closedReplayAdjudication = adjudicateClosedReplayBrokerResult(closed, replayBroker, { expectedDeployKeyId: target.id });
  requireCondition(replayChild.code === 0 && closedReplayAdjudication.valid && closedReplayAdjudication.state === "SAFE_CLOSED_REPLAY_DENIED", `C1 replay was not a safe closed denial: ${closedReplayAdjudication.reason}`);
  requireCondition(replayBroker.token_minted === false && replayBroker.provider_credential_mint_count === 0 && replayBroker.destructive_call_count === 0 && replayBroker.delete_attempted === false && replayBroker.provider_calls_after_ownership_loss === 0 && replayBroker.claim_outcome === "CLAIM_DENIED" && object(replayBroker.claim).result === "DENIED" && object(replayBroker.claim).function === "claim-effect" && object(replayBroker.claim).state === "CLOSED", "C1 replay obtained provider authority or wrong claim result");
  ...
  requireCondition(closedTerminalAuthorityProjection(terminalAgain).state === "CLOSED" && JSON.stringify(closedTerminalAuthorityProjection(terminalAgain)) === JSON.stringify(closedTerminalAuthorityProjection(closed)), "C1 replay changed terminal state");
  ...
  bundle.real_delivery.webhook_secret_value_persisted = false;
  bundle.c2_replay = { classification: c2Replay.classification, dedupe_status: c2Replay.dedupe.status, replayed: c2Replay.replayed, receipt_replay: c2Replay.receipt_replay === true, authority_rederived: c2Replay.authority_rederived, source_reads: c2Replay.source_reads, source_observation_accesses: replayObservationAccesses, active_policy_candidates: 0, incident_id: c2Replay.incident_id, create_request: c2Replay.create_request, receipt_sha256_before: receiptShaBefore, receipt_sha256_after: receiptShaAfter, new_immutable_source_reads: 0, new_t3n_incident_creations: 0, provider_authority_count: 0, provider_mutations: 0 };
  bundle.c1_replay = { remediation_reserve: sanitize(c1ReplayReserve), closed_replay_rejected: c1ReplayReserve.result !== "WON", broker: sanitize(replayBroker, [brokerKey]), closed_replay_adjudication: closedReplayAdjudication, new_effect_token_mints: 0, new_delete_count: 0, provider_calls: 0, terminal_unchanged: JSON.stringify(closedTerminalAuthorityProjection(terminalAgain)) === JSON.stringify(closedTerminalAuthorityProjection(closed)), effect_attempts: 1, target_absent: true };
  bundle.successful_policy_retirement = { ...policyRetirement, retirement_commit_sha: policyRetirementSha };
...
main().catch(async (error) => {
  ...
  console.error(JSON.stringify({ classification: "C2_E2E_R3_FAILURE", error: redact(error, [process.env.GITHUB_PAT ?? "", process.env.T3N_API_KEY ?? "", process.env.AGENT_T3N_API_KEY ?? "", process.env.EFFECT_BROKER_T3N_API_KEY ?? ""]), secret_push_issued: secretPushIssued, incident_created: incidentCreated, effect_start_confirmed: effectStartConfirmed, cleanup, no_automatic_second_run: true }, null, 2));
  process.exitCode = 1;
});
