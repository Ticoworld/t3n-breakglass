...
async function main() {
  ...
  const evidence: Record<string, unknown> = { contender: process.env.C1_CONTENDER_ID ?? "broker", broker_did: broker.did, incident_id: incidentId, input: { incident_id: incidentId, expected_claim_version: expectedClaimVersion }, ready: readyPayload(incidentId, broker.did), t3n_contract: { name: contractId, version: CONTRACT_VERSION }, token_minted: false, destructive_call_count: 0, delete_attempted: false, provider: { independent: true } };
  ...
  try { claimRaw = await invokeC1(broker.apiKey, broker.nodeUrl, contractId, "claim-effect", { incident_id: incidentId, expected_claim_version: expectedClaimVersion }); evidence.claim = claimRaw; } catch (error) { evidence.claim_error = safeError(error, [broker.apiKey]); await persist(); return; }
  const parsed = parseClaim(claimRaw);
  if (!parsed.won) { evidence.claim_outcome = "CLAIM_LOST"; evidence.token_minted = false; evidence.destructive_call_count = 0; await persist(); return; }
  ...
  try {
    ...
    if (precheckFailed || process.env.C1_INJECT_PRECHECK_FAILURE === "1") {
      ...
      const beginRaw = await invokeC1(broker.apiKey, broker.nodeUrl, contractId, "begin-effect", { incident_id: incidentId, claim_id: claim.claim_id });
      const beginResponse = json(beginRaw) as Record<string, unknown>;
      evidence.effect_start = beginRaw;
      if (beginResponse.result !== "WON" || beginResponse.function !== "begin-effect" || beginResponse.state !== "EFFECT_STARTED" || beginResponse.effect_attempts !== 1) throw new Error("begin-effect did not return a committed EFFECT_STARTED result");
      effectStarted = true;
    ...
    if (token) {
      ...
      if (revoked) { try { const probe = await repositoryRead(token, config.owner, config.repository); evidence.revoked_token_probe = { same_token: true, http_status: probe.status, refused: probe.status === 401 || probe.status === 403 }; } catch (error) { evidence.revoked_token_probe = { same_token: true, http_status: null, refused: false, transport_error: safeError(error, [token]) }; } }
    }
  }
