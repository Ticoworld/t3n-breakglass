...
import { lookupPreExistingPushPolicy, validateC2PushPolicyV2, type C2PushPolicyV2, type PushPolicyLookupOptions } from "./push-policy.js";
...
export async function processPushWebhook(
  ...
  if (dedupe.status === "DUPLICATE_SAME" && dedupe.record.state !== "RESERVED") {
    if (dedupe.record.state === "ACCEPTED" && dedupe.record.create_request && dedupe.record.derived_incident_id) {
      const nonRetiredIdentityMatches = policies.filter((candidate) =>
        candidate.policy_id === dedupe.record.policy_id && options.retiredPolicyIds?.has(candidate.policy_id) !== true,
      );
      if (nonRetiredIdentityMatches.length > 1) {
      ...
      if (policyMatches.length === 1) {
        ...
        if (!validation.valid) {
          return { classification: "C2_PUSH_REJECTED", reason: "durable replay references a policy that is no longer valid", dedupe, event };
        }
        return {
          classification: "C2_PUSH_SELECTED",
          dedupe,
      ...
      if (policyMatches.length > 1) return { classification: "C2_PUSH_POLICY_AMBIGUOUS", reason: "durable replay references a duplicated policy identity/version; refusing to select by input order", dedupe, event };
    }
    return {
      classification: "C2_PUSH_REJECTED",
